...
function filterPriceRange(min, max) {
  ...
  for (let i = 0; i < price.length; i++) {
    ...
    var name = price[i].closest("tr").getElementsByClassName("product-name");
    if (!(min < getPrice && getPrice <= max) && max != min) {
      price[i].closest("tr").style.display = "none";
      setCheckBoxForItem(name[0].innerText, false);
    } else if (max == 0 && min == 0) {
      price[i].closest("tr").style.display = "";
      checkboxes[i].checked = true;
      checkboxeAll[0].checked = true;
      setCheckBoxForItem(name[0].innerText, true);
    } else {
      price[i].closest("tr").style.display = "";
      setCheckBoxForItem(name[0].innerText, true);
      checkboxeAll[0].checked = true;
